...
namespace ACulinaryArtillery
{
    public class BlockCorkableLiquidContainer : BlockLiquidContainerTopOpened//, IContainedInteractable
    {
        ...
        //Crafting/uncrafting of corked bottles is already implemented in the DatedLiquor mod. You should be able to get it working via copying over everything plus a little extra work.

        //the oncorked and ononcorked methods are to be used later on for hooking into via the dataed liqour and other mods in the future.

        //restricting the use of a corked bottle and moving liquids with them while corked needs work.

        /*
        As for genmesh, here's what I already have planned.

        There will be three shapes in total, empty, corked, and contents all defined via attributes.

        whenever a bottle's mesh is generated:

        if the bottle is upright, it follows the following.

        get corked or empty shape based on whether it's corked or not.
        get the color of the cork top from the corkstack attribute.
        
        render the current fill level using the contents shape, which is stored as a seperate shape file and is sliced based on fill level. (to allow for non coboidal content shapes)
        
        if the bottle is sideways in a bottle rack, it does the same as before, getting the empty or corked shape, etc.

        however the slice of the contents shape is also rotated depending on the transform of the bottle item as well. (normal bottles are 90 sideways, and wine bottles are both horizontial, and at a ~70 angle leading to a triangle)

        finally I'll eventually need to add support for using a .obj or a .gtlf shape file for the contents, otherwise I wouldn't be able to use triangles.
        */
        ...
        public bool Corked => Attributes["corked"].AsBool(defaultValue: false);
        ...
        public MeshData? GenMesh(ICoreClientAPI? capi, ItemStack? contentStack, bool isSideways = false, BlockPos? forBlockPos = null)
        {
            ...
            {
                WaterTightContainableProps containableProps = BlockCorkableLiquidContainer.GetContainableProps(contentStack);
        ...
        */
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, GridRecipe byRecipe)
        {
            ...
            foreach (ItemSlot itemSlot in allInputslots)
            {
                if (itemSlot.Itemstack?.Collectible is BlockCorkableLiquidContainer)
                {
                    ...
                    outputSlot.Itemstack.Attributes.SetBool("corked", !flag);
        ...
        public override void TryMergeStacks(ItemStackMergeOperation op)
        {
            ...
            if (!Corked && op.CurrentPriority == EnumMergePriority.DirectMerge)
            {
                ...
                if (itemAttributes != null && itemAttributes["canSealBottle"]?.AsBool() == true)
                {
                    ...
                    ItemStack itemstack = op.SinkSlot.Itemstack.Clone();
                    itemstack.StackSize = 1;
                    itemstack.Attributes.SetItemstack("corkStack", new ItemStack());
                    itemstack.Attributes.SetBool("corked", true);
                    itemstack.Attributes.SetBool("canDrinkFrom", value: false);
                    itemstack.Attributes.SetBool("isTopOpened", value: false);
                    itemstack.Attributes.SetBool("allowHeldLiquidTransfer", value: false);
                    ...
                    sinkSlot.MarkDirty();
                    OnCorkContainer(op.SinkSlot, op.ActingPlayer.Entity);
                    return;
                }
            }
        ...
        public override void AddExtraHeldItemInfoPostMaterial(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world)
        {
            ...
            if (inSlot.Itemstack.Attributes.TryGetBool("corked") == true)
            {
                dsc.AppendLine("<font color=\"lightgreen\">Corked.</font>");
            };
        }
